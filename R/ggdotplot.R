...
#' @param dotsize The relative size of a dot. Smaller mean less dot overlap.
...
#' @export
ggdotplot <- function(x, binwidth=NULL, dotsize=.75){
  ...
  out <- ggplot(data, aes(x=x)) +
    geom_dotplot(binwidth = binwidth, method='histodot', dotsize=dotsize) +
    scale_y_continuous(limits=c(0, 1), expand = c(0, 0),               # Clean up the
                       breaks = seq(0, yheight, by = 2) / yheight,     # dotplot by
                       labels = seq(0, yheight, by = 2 )) +            # utilizing the
